...
# Keyboard, Linux PC type Keyboard and Tru64 Compaq Unix keyboards to mappings
# suitable for VMS
...
# Revision 1.3 19-May-1999 - fixed Linux Insert, Home, Prior, Delete and Endkey
...
if [ "$#" -eq "0" ]
then
   echo "Usage: $0 [ssh options] [host]"
   exit 1
fi
command_args=${*}
shift `expr $# - 1`
ssh_options="ssh -Y -C -2 -A"
xterm_options="-fn 9X15 -ls +t -tn vt100 +vb -geometry 132x52 -sb -T ${1} -sl 10000 -bg Black -fg Green -cr red -tn vt100 -132 "
if [ -z "$DISPLAY" ]
then
   ${ssh_options} ${command_args}
   exit 0
fi
# -n "iconicname"
...
#  char keytext[80]="This Key has been mapped to"; #  while (1) {
...
## VMS keyname "KB code required for VMS", alternate Key Name # Delete "\177 - octal 177" = BackSpace

# Top row of keys
# ---------------

## VMS keyname "KB code required for VMS", alternate Key Name or mapping # F1 "<ESC>[11~"
# F2 "<ESC>[12~"
# F3 "<ESC>[13~"
# F4 "<ESC>[14~"
# F5 "<ESC>[15~"
...
# Erase Line "<ESC>[25~" = F13, Shift/F3 # Insert/Overstrike "<ESC>[26~" = F14, Shift/F4 # HELP "<ESC>[28~" = Shift/F5,
...
## VMS keyname "KB code required for VMS", alternate Key Name or mapping # PF1 "<ESC>[OP"
...
# - string("\033") is an <ESC> seven bit character Octal=33, Decimal 27 # - <ESC>[ is equivalent to the <CSI> eight bit character Decimal 155 # - <ESC>O is equivalent to the <SS3> eight bit character Decimal 143 # - The VMS keyboard mapping codes can be found in sys$system:smgterms.txt # - these lines can also be included in a resource file if required
#   as follows:-
    ...
    ${DECXTERM_EMULATOR:=aixterm} \
      ...
      Shift<Key>KP_Add :string("\033Om")\n\
           ...
           <Key>Next :string("\033[6~")' \
             ${xterm_options} -e ${ssh_options} ${command_args} 2> /tmp/x.sh.$$.err &
    ;;

#                         -xrm 'xterm*printAttributes:1\n\
#                         -xrm 'xterm*printerCommand:lpr -P ${PRINTER:-psoce07}\n\
  HP-UX | Linux)
    xterm_options="${xterm_options} +nul -bc -pc -rightbar -hold"
    ${DECXTERM_EMULATOR:=/usr/bin/xterm} \
                         -xrm 'xterm*VT100.Translations: #override\n\
           <Key>BackSpace           :               string("\177")\n\
      Shift<Key>F1                  :string("\033") string("[23~")\n\
      ...
      Shift<Key>F10                 :string("\033") string("[34~")\n\
           <Key>F10                 :string("\033") string("[21~")\n\
       Ctrl<Key>v                   :insert-selection(PRIMARY, CUT_BUFFER1)\n\
           <Key>F12                 :print("lpr")\n\
           ...
           <Key>KP_Multiply         :string("\033") string("OR")\n\
           <Key>F26                 :string("\033") string("OR")\n\
       Ctrl<Key>KP_Subtract         :iconify()\n\
           <Key>KP_Subtract         :string("\033") string("OS")\n\
           <Key>F24                 :string("\033") string("OS")\n\
       Ctrl<Key>KP_Add              :maximize()\n\
      Shift<Key>KP_Add              :string("\033") string("Om")\n\
           <Key>KP_Add              :string("\033") string("Ol")\n\
           <Key>KP_Enter            :string("\033") string("OM")\n\
           ...
           <Key>Insert              :string("\033") string("[2~")\n\
           <Key>Home                :string("\033") string("[1~")\n\
      ~Ctrl<Key>Prior               :string("\033") string("[5~")\n\
       Ctrl<Key>Prior               :scroll-back(1,page)\n\
           <Key>Delete              :string("\033") string("[3~")\n\
           <Key>End                 :string("\033") string("[4~")\n\
      ~Ctrl<Key>Next                :string("\033") string("[6~")\n\
       Ctrl<Key>Next                :scroll-forw(1,page)\n\
           ...
           <Key>Right               :string("\033") string("[C")' \
             ${xterm_options} -e ${ssh_options} ${command_args} 2> /tmp/x.sh.$$.err &
# Shift Ctrl<Key>V                   :Insert:insert-selection(CLIPBOARD, CUT_BUFFER0, PRIMARY)\n\
    ;;
  ...
  *)
   ...
   if [ -s "/tmp/x.sh.$$.err" -a -f "/tmp/x.sh.$$.err" ]
   then
      echo "Previously suppressed errors occurred during session:"
      cat /tmp/x.sh.$$.err
      rm -f /tmp/x.sh.$$.err
   fi
   exit 0
